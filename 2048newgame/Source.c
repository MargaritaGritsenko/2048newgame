...
void SaveGame(PDISPLAY GameBoard);
void Game();
void PrintSave(PDISPLAY GameBoard);
void GameSave();
...
void PrintBoard(PDISPLAY GameBoard)  // Отображение поля
{
    ...
    printf("\n \tТекущий счет:%d", GameBoard->score);
    printf("\n-----------------------------------\n\n");
...
void SaveGame(PDISPLAY GameBoard)
{
    char fname[20] = "SaveGame.txt";
    FILE* save;
    int score = GameBoard->score;
    if ((save = fopen(fname, "wt")) == NULL)
    {
...
void PrintSave(PDISPLAY GameBoard)
{
    char fname[20] = "SaveGame.txt";
    FILE* save;
    if ((save = fopen(fname, "rt")) == NULL)
    {
        printf("Ошибка открытия файла");
        return 0;
    }
    fscanf(save, "%d", &GameBoard->score);
    for (int i = 0; i < SIZE_BOARD; i++)
    {
        for (int j = 0; j < SIZE_BOARD; j++)
        {
            fscanf(save, "%d", &GameBoard->board[i][j]);
        }
    }
    fclose(save);
    return 1;
